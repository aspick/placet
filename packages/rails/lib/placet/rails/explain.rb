...
module Placet
  module Rails
    # rails placet:explain の整形ロジック（rake タスクから独立してテストできる）。
    # resolver はアプリの user オブジェクトを必要とするため、explain は principal
    # 集合を直接受け取り、derive の 1 段展開だけを適用する
    module Explain
      module_function
      ...
      def decision_report(principals, action)
        decision = Placet.engine.decide(Placet.expand_principals(principals), action)
